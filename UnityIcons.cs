...
using System.Linq;
using Tools.EditorDrawer;
using Tools.EditorDrawer.Editor;
using UnityEditor;
using UnityEngine;
...
namespace Tools.Editor
{
	public class UnityIcons : EditorWindow
	{
		...
		private GUIStyle boxStyle => m_BoxStyle ?? (m_BoxStyle = new GUIStyle("HelpBox")
		{
		...
		private GUIStyle centeredStyle => m_CenteredStyle ?? (m_CenteredStyle = new GUIStyle("Label")
		{
			alignment = TextAnchor.MiddleCenter
		});
		...
		private GUIStyle rightStyle => m_RightStyle ?? (m_RightStyle = new GUIStyle("Label")
		{
		...
		private void OnGUI()
		{
			...
			string result = EditorHelper.SearchField(m_SearchFilter);
		...
		{
			using (GUIScope.ScrollView(ref _scroll))
			{
				using (GUIScope.GridScope grid = GUIScope.Grid(Mathf.FloorToInt(position.width / _itemSize.x), _itemSize))
				{
		...
		private void DrawStyle(Rect _rect, GUIStyle _style)
		{
			...
			SplitResult split = GUISplit.SplitHorizontal(_rect, -1f, -1f, -1f);

			if (Event.current.type == EventType.Repaint)
			{
				_style.Draw(split.Next(), "Inactive", false, false, false, false);
				_style.Draw(split.Next(), "Active", false, true, false, false);
				_style.Draw(split.Next(), "Pressed", false, false, true, false);
			}
			...
			{
				GUIUtility.systemCopyBuffer = $"(GUIStyle)\"{_style.name}\"";
